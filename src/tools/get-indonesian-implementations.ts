/**
 * get_indonesian_implementations — Find Indonesian statutes that reference a specific EU directive/regulation.
 */

import type Database from '@ansvar/mcp-sqlite';
import { generateResponseMetadata, type ToolResponse } from '../utils/metadata.js';

export interface GetIndonesianImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface IndonesianImplementationResult {
  document_id: string;
...
export async function getIndonesianImplementations(
  db: InstanceType<typeof Database>,
  input: GetIndonesianImplementationsInput,
): Promise<ToolResponse<IndonesianImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as IndonesianImplementationResult[];
