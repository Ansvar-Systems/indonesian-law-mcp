/**
 * search_eu_implementations — Search EU directives/regulations referenced by Indonesian legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_indonesian_implementation?: boolean;
  limit?: number;
}
...
export interface EUImplementationSearchResult {
  ...
  indonesian_statute_count: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as indonesian_statute_count
  ...
  if (input.has_indonesian_implementation) {
    sql += ' HAVING indonesian_statute_count > 0';
  }
