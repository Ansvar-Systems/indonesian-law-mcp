/**
 * Tool registry for Indonesian Law MCP Server.
...
import { getIndonesianImplementations, type GetIndonesianImplementationsInput } from './get-indonesian-implementations.js';
...
const LIST_SOURCES_TOOL: Tool = {
  ...
  description:
    ...
    'including the Federal Register of Legislation (Indonesian Government, Office of Parliamentary Counsel). ' +
    ...
    'Call this FIRST when you need to understand what Indonesian legal data this server covers.',
  inputSchema: { type: 'object', properties: {} },
};
...
export const TOOLS: Tool[] = [
  {
    ...
    description:
      'Search Indonesian statutes and regulations by keyword using full-text search (FTS5 with BM25 ranking). ' +
  ...
  },
  {
    name: 'get_provision',
    description:
      'Retrieve the full text of a specific provision (section) from an Indonesian statute. ' +
  ...
  },
  {
    ...
    description:
      'Validate an Indonesian legal citation against the database — zero-hallucination check. ' +
  ...
  },
  {
    ...
    description:
      'Build a comprehensive set of citations for a legal question by searching across all Indonesian statutes simultaneously. ' +
      'Returns aggregated results from multiple relevant provisions, useful for legal research on a topic. ' +
      'Use this for broad legal questions like "What are the penalties for data breaches in Indonesia?" ' +
  ...
  },
  {
    ...
    description:
      'Format an Indonesian legal citation per standard conventions. ' +
  ...
  },
  {
    name: 'check_currency',
    description:
      'Check whether an Indonesian statute or provision is currently in force, amended, repealed, or not yet in force. ' +
  ...
  },
  {
    name: 'get_eu_basis',
    description:
      'Get the EU legal basis that an Indonesian statute references or aligns with. ' +
      'Indonesia is not an EU member but Indonesian laws may reference EU directives/regulations ' +
    ...
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Indonesian statute identifier.' },
        include_articles: {
          type: 'boolean',
  ...
  {
    name: 'get_indonesian_implementations',
    description:
      'Find all Indonesian statutes that reference or align with a specific EU directive or regulation. ' +
      'Given an EU document ID (e.g., "regulation:2016/679" for GDPR), returns matching Indonesian statutes. ' +
      'Note: Indonesia does not transpose EU law but may reference or align with EU frameworks voluntarily.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description:
      'Search for EU directives and regulations that are referenced by Indonesian legislation. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_indonesian_implementation: {
          type: 'boolean',
          description: 'If true, only return EU documents referenced by Indonesian legislation.',
        },
  ...
  },
  {
    ...
    description:
      'Get the EU legal basis for a SPECIFIC provision within an Indonesian statute. ' +
  ...
  },
  {
    ...
    description:
      'Check EU alignment status for an Indonesian statute or provision. ' +
      ...
      'Note: Indonesia is not bound by EU law; this checks voluntary alignment and cross-references.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Indonesian statute identifier.' },
    ...
    try {
      ...
      switch (name) {
        ...
        case 'get_eu_basis':
          ...
          break;
        case 'get_indonesian_implementations':
          result = await getIndonesianImplementations(db, args as unknown as GetIndonesianImplementationsInput);
          break;
