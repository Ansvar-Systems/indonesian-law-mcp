#!/usr/bin/env tsx
/**
 * Indonesian Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'https://jdih.setneg.go.id';
const PORTAL_NAME = 'JDIH (Jaringan Dokumentasi dan Informasi Hukum)';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/indonesian-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Indonesian Law MCP — Data Freshness Check');
